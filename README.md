...
<p align="center">
  ...
  <a href="#translated-documentation"><img src="https://img.shields.io/badge/Translated%20READMEs-24%20(headings%20only)-lightgrey.svg" alt="24 translated READMEs, headings only" /></a>
</p>
...
> **Author & Web Developer:** Arnold Wender ([ORCID: 0009-0005-1750-818X](https://orcid.org/0009-0005-1750-818X)) · Inhaber, Wender Media (Einzelunternehmen) · Halle (Saale), Germany · 2026  
...
php tests/test-pot-freshness.php # every string the code translates has an entry in the .pot (needs wp-cli)
...
- **The catalogs translate almost nothing, and the old measurement overstated it.** Until 2026-09-20, 118 of the 123 translatable strings had no entry in the `.pot` at all, so no translator could see them in any language. The catalog was regenerated from the code (`tests/test-pot-freshness.php` now keeps it that way) and the honest count is **4 translated entries across the 28 locales**. The earlier figure — `ar` at 70 of 70 — was measured against the old catalog: **69 of those 70 translate strings of the Spanish settings page this plugin no longer has** (`-- Seleccionar proporción o escribir valor --`), so they had already stopped appearing anywhere. The 2,007 entries that no longer match a string are kept as obsolete `#~` entries, not deleted. Every locale shows the German source strings of the admin screens.
...
`languages/` carries `.po` / `.mo` files for the 24 official EU languages (28 locale files); they are not translated yet, see Known issues. Translated READMEs live in `docs/translations/README.<lang>.md`: **their headings are translated and their bodies carry the English text of this file** (generated by `wp-plugin-fleet-translations-sync.py`), so they are a translated table of contents over an English document, not 24 translated manuals. The badge above says so; it read "Translations — 24 EU Languages" in green until 2026-09-20, which a reader takes as the plugin being translated.
