...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/wm-modularscale-mutations.XXXXXX")
trap 'rm -rf "$WORK"' EXIT

RESULTS_GREEN='^ WM MODULAR SCALE TEST RESULTS: [1-9][0-9]* PASSED \| 0 FAILED$'
RESULTS_RED='^ WM MODULAR SCALE TEST RESULTS: [0-9]+ PASSED \| [1-9][0-9]* FAILED$'
...
selfcheck_out=" WM MODULAR SCALE TEST RESULTS: 1 PASSED | 1 FAILED"$'\n'"$(head -c 1048576 /dev/zero | tr '\0' 'x')"
